Shader "Example/Unlit/ParallaxHeight2"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float2 ParallaxFunc(float2 uv, float3 view)
			{
				...
				float PreDepthValue = tex2Dlod(_HeightTex, (uv + Deltauv).xyxy);
				float DeltaCurDepth = CurDepthValue - CurDepth;
				float DeltaPreDepth = PreDepthValue - (CurDepth - DeltaDepth);

				uv = uv + Deltauv * DeltaCurDepth / (DeltaCurDepth - DeltaPreDepth);
